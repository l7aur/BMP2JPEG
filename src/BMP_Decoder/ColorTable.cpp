...
Util::Pixel4 ColorTable::at(const uint8_t i) const {
    const int index = 4 * i;
    if (index > tableSize)
        return Util::Pixel4{0xFF, 0xFF, 0xFF, 0xFF};
    return Util::Pixel4{data[index + 2], data[index + 1], data[index], data[index + 3]};
}
